...
class RepoPinImgMedia:
    ...
    def __init__(
        ...
        self.__bg_img_mime: enums.RepoPinsImgMediaBgImgMime = enums.RepoPinsImgMediaBgImgMime.PNG
        self.__bg_img_align: enums.RepoPinsImgMediaBgImgAlign = (
            enums.RepoPinsImgMediaBgImgAlign(align.lower())
            if align
            and align.lower() in enums.RepoPinsImgMediaBgImgAlign
        ...
        self.__bg_img_opacity: float = opacity if opacity and 0 <= opacity <= 100 else self.__DEFAULT_OPACITY
    ...
    def __format_encoded_img(self, raw_img: bytes) -> bytes:
        ...
        img_encoder: im.Image = im_ops.exif_transpose(image=im.open(fp=BytesIO(initial_bytes=raw_img)))
    ...
    def __set_encoded_url(self, encoded_url: str) -> None:
        self.__bg_img_encoded_url = f"data:{self.__bg_img_mime.value.lower()};base64,{encoded_url}"
    ...
    def __load_url(self) -> None:
        try:
            ...
            raise RepoPinImageMediaError(msg=f"HTTP error attempting to fetch background image URL.")
    ...
    def __load_file(self) -> None:
        try:
            self.__bg_img_mime = enums.RepoPinsImgMediaBgImgMime(
                [
                    e.value for e in enums.RepoPinsImgMediaBgImgMime
